...
> [!WARNING]
> **LLM 지출은 의도적으로 제한한다.** 프로덕션 점수 채점은 실행당 180쌍 상한과
> 계정 $5 상한이 있고, 과거 기사 백필은 승인되지 않았다. 합성(⑤/⑧)은
> `config/models.yaml`에서 비활성화했으므로 Anthropic 호출을 하지 않는다. 다시 켜기,
> 상한 확대, 유료 백필은 모두 Ricky의 명시적 예산 승인이 필요하다.
> `news_polarity`는 2026-11-13 게이트 뒤까지 등급 합성에서 제외된다. 현재 운영 순서:
...
> [!NOTE]
> 2026-08-27 리뷰가 찾은 우회 사례는 2026-08-28 fail-closed 수정으로 닫았다. 검증 불가 등급 표현은 섹션을 생략하고, 미등급 종목 뒤에는 이전 종목 주체를 지우며, 저가매수 같은 검토된 복합 추천어도 잡는다. 현재 작업 트리는 ⑤·⑧을 벤더 호출 전에 비활성화하므로 Anthropic secret 등록은 필요하지도 승인되지도 않았다. [notes/review-2026-09-05.md](notes/review-2026-09-05.md) 참고.
...
**GitHub cron은 최선 노력이었고, 저장된 기록이 그 손실을 측정했다.** 2026-08-03~07 동안 뉴스 워크플로가 하루 31회를 선언하고 6~10회를 전달해 시간당 커버리지 42.6%였다. 현재 작업 트리는 cron 선언을 버전 관리되는 Cloudflare Worker dispatcher와 watchdog으로 바꾼다. 다만 **아직 배포되지 않았으므로**, Worker secret과 배포가 끝날 때까지 GitHub가 라이브 스케줄러다. 하류는 여전히 공백이 실제로 무엇을 잃었는지 추측 대신 측정한다. [next-steps-2026-09-05.md](notes/next-steps-2026-09-05.md) 참고.
...
**`src/embed/`** — 구축·실측 완료됐으나 **일일 실행에 미연결.** 중복 제거 절반(제목 코사인 > 0.85)과 보류된 topicality 절반은 `tests/`에서만 import되고, 프로덕션 파이프라인은 (기사, 종목) 후보를 `src/entity/resolve.py`에서 바로 받는다 — 그것만으로 SPEC §6.1의 60~100 목표 구간 안에 든다. 이것은 `news_polarity`를 막지 않는다. 스코어링은 프로덕션 증거가 있고, 가중치만 게이트까지 동결돼 있다.
...
```bash
uv sync                          # 의존성 설치
uv run pytest -m "not network"   # 기본 테스트 실행 — 925개
...
**스코어링과 복구된 공시 경로에는 프로덕션 증거가 있다.** 2026-08-29 수동 리포트 실행은 536쌍을 채점·커밋하고, 아카이브에서 ③을 렌더했으며, 가드가 통과한 문장과 두 공시 수집기 복구를 확인했다. 현재 작업 트리는 Anthropic 지출을 막기 위해 선택적 ⑤/⑧ 합성을 비활성화한다. 스코어링 상한과 `news_polarity`의 동결된 등급 가중치는 유지한다. 남은 운영 항목은 코드·테스트가 끝났지만 아직 배포되지 않은 Cloudflare 스케줄러 전환이다.
...
| 3 | 수집기 + 검증 테스트 | ✅ 수집기 10개, 오프라인 925 / 네트워크 20. 공시 자격증명과 비거래일 fallback은 프로덕션 증거가 있음 |
...
| 10 | 리포트 렌더러 + 전달 | 🟡 vault + 이메일 가동. ③ 점수 집계와 가드가 적용된 문장은 프로덕션 증거가 있고, 현재 작업 트리는 Anthropic 호출 전 ⑤/⑧을 비활성화 |
| 11 | 일일 수집 + 리포트 워크플로 | 🟡 점수 아카이브와 공시 복구는 CI에서 확인. Cloudflare 스케줄러 전환은 코드 준비 완료, Ricky의 Worker/PAT 설정 대기 |
| 12 | 스케줄 번인 | ✅ 과거 게이트 창 완료; 이후 실행 전달은 보류 중인 Cloudflare 전환에 달림 |
...
미결 항목은 [MANUAL-TASKS.md](MANUAL-TASKS.md)에 무엇을 막는지 순서로 추적된다. 현재 작업 트리 기준:

| 할 일 | 막는 것 | 주체 |
|---|---|---|
| 스케줄러·비용 제어 변경 검토 및 반영 | 테스트된 Worker 소스와 fail-closed 합성 정책을 배포 코드로 만듦 | Ricky + Codex |
| Cloudflare Free Worker + 90일 `Actions: write` PAT 생성 | GitHub cron 제거 뒤 수집/리포트 dispatch 복구 | Ricky |
| 자연 발생 뉴스 dispatch 1회, 각 리포트 슬롯 확인 | 유료 리포트 테스트 없이 운영 증거 확보 | Ricky + Codex |
| 과거 뉴스 점수 백필을 승인하지 않음 | 별도 명시적 승인 전 무비용 정책 유지 | Ricky |
...
인프라 판단은 dispatch 전용 전환이다. Cloudflare가 시계만 맡고 GitHub Actions가
수집기와 리포트를 계속 실행한다. Worker와 제한된 자격증명 저장소가 추가되지만,
파이프라인 실행·데이터 저장을 옮기지 않고 기록된 RSS 유실 위험을 다룬다.
...
| [2026-08-27](notes/review-2026-08-27.md) | 과거 리뷰: 프로덕션 배선 실패와 코드 발견 |
| [2026-09-05](notes/review-2026-09-05.md) | **현재 리뷰.** Cloudflare 전환 코드, 무비용 합성 정책, 수동 배포 경계 |
