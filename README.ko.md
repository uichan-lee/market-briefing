...
2026-08-03부터 무인으로 돌고 있다. [`reports/`](reports/)에 25개의 브리핑이 커밋돼 있고, 샘플이 아니라 실제 출력이다.
...
**가동 단계.** 결정론적 파이프라인이 수집·해석·계산·등급·렌더·전달을 하루 두 번 무인으로 수행하고 있고, 2026-08-03부터 그래 왔다. `data/raw/`에 3년치 백필과 라이브 뉴스 기록이 있다. 골든셋과 모델 베이크오프는 끝났다. 빠진 것은 임베딩 파이프라인이고, 그게 없으면 `news_polarity`가 채점할 대상이 없다. §2.2④(캘린더)는 2026-08-14에 전면 부재에서 부분 구현으로 바뀌었다 — CPI·고용지표·FOMC 발표일과 옵션 만기가 실제 수집 데이터가 됐다. 미국 개별 종목 실적 발표일과 한국 배당락·IPO 일정은 여전히 명시적으로 부재이고, §2.2⑥ 방향성 등급도 여전히 KR 31종목 전용이다 — 둘 다 실수가 아니라 의도적 결정이다. [notes/calendar-collector-plan.md](notes/calendar-collector-plan.md), [notes/us-rating-plan.md](notes/us-rating-plan.md) 참고.
...
| 10 | 리포트 렌더러 + 전달 | ✅ vault + 이메일 가동, 브리핑 25건 렌더 |
| 11 | 일일 수집 + 리포트 워크플로 | ✅ 2026-08-06 클라우드 왕복 완주 |
| 12 | 스케줄 번인 | 🟡 진행 중 — cron이 무인으로 발사 중 |
| 13 | 2주 게이트 | 🟡 4번 기준(모델간 polarity 상관)은 2026-08-15에 측정, **통과**(0.5 기준 대비 0.81~0.88) — [PREREGISTRATION §R](PREREGISTRATION.md). 1~3번 기준은 2026-08-26에 판독(원래 08-25 고정에서 하루 밀림 — [PREREGISTRATION §8.5](PREREGISTRATION.md)); `src/eval/gate_2week.py report`로 실제 데이터 대상 사전 확인 가능 |
...
**`rev_4w`에 데이터 소스가 없다.** [SPEC §5](SPEC.md)가 이를 **컨센서스** EPS의 4주 변화 — 즉 포워드 애널리스트 추정 — 로 정의한다. pykrx의 `EPS`는 후행이고, 그것으로 대체하면 피처처럼 보이지만 피처가 아닌 숫자가 나온다. 제대로 하려면 추정치 벤더가 필요하다. 가중치 0.15이고, 없으면 `rate()`가 재정규화한다. 벤더 조사는 2026-08-14 기준 끝났다 — FnGuide/QuantiWise는 기업 상담 전용이고 공개된 개인 요금제가 없으며, 무료 컨센서스 페이지 9곳을 스윕한 결과 전부 스크래핑이 구조적으로 막혀 있었다(robots.txt 또는 명시적 약관 금지). 대학 접근 경로(WRDS/FactSet/Capital IQ Pro)는 **닫혔다** — Haas 도서관의 2026-08-21 답변이 대량 다운로드는 원칙적으로 금지고 졸업 후 라이선스 데이터 보관도 금지라고 확인해서, 이 경로가 쓸모 있으려면 필요했던 유일한 시나리오(한 번 백필해서 영구 보관)가 막혔다 — 모든 걸 영구 보관하는 이 프로젝트엔 안 맞는다. 대학 신분과 무관한 유일한 유료 경로는 FnSpace Academy 요금제(월 5만원)로 남아 있다 — [notes/rev4w-vendor-research.md](notes/rev4w-vendor-research.md) 참고.
