...
2026-08-03부터 무인으로 돌고 있다. [`reports/`](reports/)에 30개가 넘는 브리핑이 커밋돼 있고, 샘플이 아니라 실제 출력이다.

> [!WARNING]
> **LLM 단계는 코드상 완성됐지만 프로덕션에서 돈 적이 없다.** ⑤(반증)·⑧(AI 총평)은 2026-08-25에 `render()`에 배선됐고, `news_polarity` 스코어링은 2026-08-27에 일일 실행에 배선됐다. 2026-08-28 로컬 변경은 자격증명 4개를 `report.yml`에 매핑하고, `data/scores/`를 커밋하며, 유료 호출을 중간 저장하고, 총평 가드 우회를 닫고, 저장된 점수로 ③을 렌더한다. `OPENAI_API_KEY`·`ANTHROPIC_API_KEY`는 아직 Actions secret이 아니고 이 변경은 프로덕션 실행을 마치지 않았다. 따라서 현재 커밋된 브리핑은 여전히 결정론적 수급·시세 모델뿐이다. 현재 순서: [notes/next-steps-2026-08-28.md](notes/next-steps-2026-08-28.md).
>
> **이 미커밋 변경이 반영되기 전에는 키를 등록하지 않는다.** 가드, 점수 커밋·중간
> 저장, ③ 점수 집계는 구현됐고 오프라인 테스트 921개를 통과했지만, 프로덕션 증거는
> 변경 반영 뒤 Actions 실행 한 번이 더 필요하다. `news_polarity`는 2026-11-13
> 게이트 뒤까지 등급 합성에서 제외된다.
...
(`rev_4w`, 가중치 0.15는 2026-08-25에 영구 드랍됐다 — 무료이면서 약관에 저촉되지 않는
컨센서스 EPS 소스가 없다. [SPEC §5](SPEC.md)와
[notes/rev4w-vendor-research.md](notes/rev4w-vendor-research.md) 참조. 설계 가중치
합계는 이제 1.10이 아니라 0.95다.)
...
> [!NOTE]
> 2026-08-27 리뷰가 찾은 우회 사례는 2026-08-28 fail-closed 수정으로 닫았다. 검증 불가 등급 표현은 섹션을 생략하고, 미등급 종목 뒤에는 이전 종목 주체를 지우며, 저가매수 같은 검토된 복합 추천어도 잡는다. ⑤·⑧은 여전히 Ricky의 Actions secret 등록을 기다린다. 상세는 [notes/review-2026-08-27.md](notes/review-2026-08-27.md) §2 F1.
...
| 게이트 | 기준 | 미달 시 |
|---|---|---|
| **2주** ✅ **통과 2026-08-27**<br>2026-08-12 → 08-26 | 파이프라인 무중단 · 데이터 정합성 오류 0 · `ambiguous` < 30% · 모델간 polarity 상관 > 0.5 | 시그널 작업 중단, 측정 계층 수리 |
...
네 기준 모두 충족으로 판독됐다: `ambiguous` 약 9%, 계수된 데이터 정합성 오류 0, 실행 무중단, 모델간 polarity 상관 0.81~0.88 ([PREREGISTRATION §R](PREREGISTRATION.md), 2026-08-27; 4번 기준은 2026-08-15 측정). 이 게이트는 시그널이 아니라 **측정 계층**을 검증하며, LLM 단계가 2026-08-25부터 CI에서 죽어 있는 파이프라인 위에서 통과했다(위 경고 참조).
...
│   ├── filing_ids.yaml        KR 공시 수집기용 DART 고유번호
│   └── delivery.yaml          출력 채널 — 채널을 선언할 수 있는 유일한 곳
│
├── src/
│   ├── util/session.py        UTC/KST, 거래일, 룩어헤드 경계
│   ├── util/config.py         설정 로딩 + 손편집 안전장치
│   ├── collectors/            validate.py + 수집기 10개 (KR/US 시세, KR 수급/지수, KR/US 공시, 뉴스, 매크로, 캘린더)
│   ├── entity/resolve.py      별칭 기반 종목 매칭 + 모호 버킷
│   ├── embed/                 중복 제거(2026-08-15 실측) + 관련성(topicality, 2026-08-22 실측, 배포 보류) — 둘 다 일일 실행에 미연결
│   ├── features/              compute.py (피처 6개: 활성 5 + news_polarity 보류) + normalize.py (z-score)
│   ├── llm/                   adapter.py (벤더 중립), score.py, daily_scoring.py, synthesize.py, prompts/
...
├── reports/                   렌더된 브리핑, 매일 커밋 (30건 이상)
├── tests/                     오프라인 921, 네트워크 20
└── data/                      raw/ · ratings/ · bakeoff/ · golden/ · scores/ 를 .gitignore가 예외 처리 — 각각의 이유는 그 파일에
```

<details>
<summary><b>완성된 모듈이 실제로 하는 일</b></summary>

**`src/embed/`** — 구축·실측 완료됐으나 **일일 실행에 미연결.** 중복 제거 절반(제목 코사인 > 0.85)과 보류된 topicality 절반은 `tests/`에서만 import되고, 프로덕션 파이프라인은 (기사, 종목) 후보를 `src/entity/resolve.py`에서 바로 받는다 — 그것만으로 SPEC §6.1의 60~100 목표 구간 안에 든다. 즉 "임베딩 파이프라인"은 `news_polarity`를 막는 것이 아니다 — CI의 LLM 스코어링 자격증명이 막는다.
...
```bash
uv sync                          # 의존성 설치
uv run pytest -m "not network"   # 기본 테스트 실행 — 902개
...
**가동 단계, 2주 게이트 통과 2026-08-27.** 결정론적 파이프라인이 수집·해석·계산·등급·렌더·전달을 하루 두 번 무인으로 수행하고 있고, 2026-08-03부터 그래 왔다. `data/raw/`에 3년치 백필과 라이브 뉴스 기록이 있다. 골든셋과 모델 베이크오프는 끝났다.

**아직 프로덕션에서 증명되지 않은 것: LLM 절반과 복구된 공시 경로.** `src/llm/daily_scoring.py`(뉴스 → `news_polarity`)와 `src/llm/synthesize.py`(§2.2⑤/⑧)는 일일 실행에 배선돼 있다. 2026-08-28 미커밋 변경은 워크플로 자격증명, `data/scores/` 보존, 점수 중간 저장, ③ 집계, 비거래일 공시 시각을 고쳤다. `OPENAI_API_KEY`·`ANTHROPIC_API_KEY`는 여전히 Actions secret이 아니므로, 실제 활성화와 첫 점수 아카이브 확인은 남았다. 임베딩 파이프라인은 블로커가 아니다(`daily_scoring.py`는 `src/entity/resolve.py`를 직접 읽는다).

§2.2④(캘린더)는 2026-08-14에 전면 부재에서 부분 구현으로 바뀌었다 — CPI·고용지표·FOMC 발표일과 옵션 만기가 실제 수집 데이터가 됐다. 미국 개별 종목 실적 발표일과 한국 배당락·IPO 일정은 여전히 명시적으로 부재이고, §2.2⑥ 방향성 등급도 여전히 KR 31종목 전용이다 — 둘 다 실수가 아니라 의도적 결정이다. [notes/calendar-collector-plan.md](notes/calendar-collector-plan.md), [notes/us-rating-plan.md](notes/us-rating-plan.md) 참고.
...
| 3 | 수집기 + 검증 테스트 | ✅ 수집기 10개, 오프라인 921 / 네트워크 20. 공시 자격증명과 비거래일 fallback은 로컬에서 수정됐고 첫 프로덕션 확인은 11단계에 남음 |
| 4 | `data/raw/`로 3년 백필 | ✅ macro · us_price · kr_price · kr_flow, 2023-08-03 → 2026-08-07 |
| 5 | 엔티티 해석 + 모호 비율 | ✅ 게이트 창 전체에서 모호 약 9% (임계 30%) |
| 6 | 임베딩 파이프라인 (중복 제거 + 관련성) | 🟡 중복 제거 절반은 2026-08-15에 구축·실측 완료(title-only, cos>0.85). 관련성(topicality) 절반은 2026-08-22에 실제 라벨 149건으로 실측 — 결과는 부정적: AUC 0.74지만 두 클래스가 너무 겹쳐서 어떤 기준값도 "전부 topical" 베이스라인(정확도 0.752)을 못 이김. **1단계 topicality 필터는 배포 보류**, `src/embed/topicality.py`는 테스트된 채로 파이프라인 미연결 — [notes/step6-plan.md](notes/step6-plan.md). 두 절반 다 일일 실행에 미연결이고, 아무것도 막지 않음 |
| 7 | 골든셋 — 손수 라벨링한 기사 100건 | ✅ 완료. `relevance`는 정의 변경 후 2026-08-12에 전량 재라벨링 |
| 8 | 모델 어댑터 + 베이크오프 | ✅ 2026-08-13 완료 — `gpt-5.4` 선택 |
| 9 | 피처 계산 | ✅ 활성 등급 피처 5개(가중치 0.75) + `news_polarity` producer 2026-08-27 구축, 가중치 보류. 설계 합계 0.95 (`rev_4w` 드랍 전엔 1.10) |
| 10 | 리포트 렌더러 + 전달 | 🟡 vault + 이메일 가동, 브리핑 30건 이상 렌더. ③ 점수 집계와 fail-closed ⑤·⑧ 가드는 구현됐고, LLM 출력은 Actions secret과 프로덕션 확인 대기 |
| 11 | 일일 수집 + 리포트 워크플로 | 🟡 2026-08-06 클라우드 왕복 완주. 미커밋 변경은 자격증명 4개, `data/scores`, 호출 중간 저장, 세션 기준 파일명을 반영했고 merge + 첫 CI 확인이 남음 |
| 12 | 스케줄 번인 | ✅ 게이트 창 전체를 cron이 무인으로 통과 |
| 13 | 2주 게이트 | ✅ **통과 2026-08-27** — 4번 기준 2026-08-15 측정(0.5 기준 대비 0.81~0.88), 1~3번 기준은 `src/eval/gate_2week.py`로 창 전체 판독 ([PREREGISTRATION §R](PREREGISTRATION.md), 2026-08-27) |
...
**`rev_4w`는 2026-08-25에 영구 드랍됐다.** [SPEC §5](SPEC.md)는 이를 **컨센서스** EPS(증권사 전망치)의 4주 변화로 정의했다. pykrx의 `EPS`는 후행이고, 그것으로 대체하면 피처처럼 보이지만 피처가 아닌 숫자가 나온다. 2026-08-14 벤더 스윕에서 무료이면서 약관에 저촉되지 않는 소스를 못 찾았다 — FnGuide/QuantiWise는 기업 상담 전용, 무료 컨센서스 페이지 9곳 전부 스크래핑 구조적 차단, 대학 경로(WRDS/FactSet/Capital IQ Pro)는 Haas 도서관이 2026-08-21에 닫힘을 확인(대량 다운로드 금지, 졸업 후 보관 금지). FnSpace Academy 요금제(월 5만원 ≈ $36)가 남은 유일한 유료 경로였고 Ricky의 비용선을 넘어, `rev_4w`는 보류가 아니라 `config/rating.yaml`에서 완전히 제거됐다. `rate()`는 `deferred_weights`를 안 읽으므로 계산된 등급은 안 바뀐다. 설계 가중치 합계: 0.95. [notes/rev4w-vendor-research.md](notes/rev4w-vendor-research.md), [MANUAL-TASKS §11](MANUAL-TASKS.md) 참고.
...
### 결정된 것 하나: 미국 종목은 당분간 §2.2⑥ 범위 밖

SPEC §2.2⑥은 "모든 워치리스트 종목이 등급을 받는다"고 적혀 있었지만, `render.py`의 등급 계산 경로는 항상 KR 31종목에 고정돼 있었다 — 미국 40종목은 §2.2①(지수·섹터 단위 전이)에만 노출됐다. 2026-08-14에 발견하고 그 자리에서 확정했다. 지금 `rate()`를 미국 종목까지 넓히면(활성 가중치가 미국엔 없는 한국 투자자별 수급 데이터라서) 전부 `min_weight_coverage` 밑으로 떨어져 근거 없이 관망으로 강제되는데, 이건 정직한 공백보다 나쁘다. 미국 개별 종목 등급은 별도 피처 세트가 필요하고, KR 파이프라인이 2주·3개월 게이트를 통과할 때까지 미룬다. 전체 근거는 [notes/us-rating-plan.md](notes/us-rating-plan.md)에.

### 왜 9단계를, 그리고 10단계까지 6~8단계보다 먼저 하나

`news_polarity`(0.20)는 LLM이 필요한 유일한 등급 피처다. 9단계에서 만든 다섯 개가 총 0.95 중 0.75를 지고 있어 `min_weight_coverage: 0.5`를 넘으므로, 임베딩 파이프라인·골든셋·베이크오프 없이도 진짜 결정론적 등급에 도달할 수 있다. 순서를 뒤집은 이유가 그것이다.
...
미결 항목은 [MANUAL-TASKS.md](MANUAL-TASKS.md)에 무엇을 막는지 순서로 추적된다. 2026-08-28 미커밋 변경 기준:

| 할 일 | 막는 것 | 주체 |
|---|---|---|
| 미커밋 가드·워크플로·스코어링·렌더러 변경 검토, rebase, 반영 | 테스트된 안전장치를 프로덕션 코드로 만듦 | Ricky + Codex |
| 변경 반영 뒤 `OPENAI_API_KEY` + `ANTHROPIC_API_KEY`를 Actions secret으로 등록 | 모든 LLM 스코어링, §2.2⑤, §2.2⑧, `news_polarity` 체인 전체 | Ricky |
| Actions 실행 한 번에서 공시·점수 저장·③ 집계·⑤/⑧ 가드를 확인 | 오프라인 테스트가 증명하지 못하는 secret과 artifact 보존 확인 | Codex |
| 보존 확인 뒤 과거 뉴스 점수 백필 승인 | 유료 호출 중복 위험 없이 `news_polarity` 이력 구축 | Ricky |
| `config/rating.yaml` 캘리브레이션 | 신뢰할 수 있는 등급 — 2026-08-23 예정, **지연** | Ricky (MANUAL-TASKS §6) |
| `valuation_band` 백필 결정 | 하드 블로킹 아님; 2026-09-11에 자동 해소 | Ricky (MANUAL-TASKS §11) |

닫힘: ~$5 게이트 측정 비용(2026-08-15, 통과); `rev_4w`(2026-08-25, 드랍); KIS 키(2026-08-05, 보유, 미소비).

**6단계는 아무것도 막지 않고 아무것도 6단계를 막지 않는다** — `src/entity/resolve.py`만으로 이미 하루 92~148개 (기사, 종목) 쌍, SPEC §6.1의 60~100 구간 안.

2026-11-13 게이트 전에는 기록된 결함 네 가지도 닫아야 한다.
`kr_filings`가 KRX 휴장일 공시를 처리해야 하고, 섀도 포트폴리오는 낮은 근거 충족도로
관망 강제된 종목을 제외해야 하며, 리포트 파일명은 늦게 뜬 러너의 읽는 날짜가 아니라
해석된 세션을 따라야 하고, raw revision 비교는 동일한 calendar/filings `-vN` 파일을
계속 만들지 않아야 한다. 현재 등급 산술은 바꾸지 않지만, 섀도 결함은 남겨두면 3개월
P&L 판독을 무효화할 수 있다.

인프라 판단은 전면 이전이 아니다. 실현된 RSS 유실이 낮은 수집은 GitHub Actions에
남기고, 여러 cron이 함께 수시간 지연되는 현상이 반복되면 하루 두 번의 **리포트
트리거만** 외부 스케줄러로 분리할지 재검토한다. 대가는 별도 호스트·자격증명 저장소·
배포 표면이다.
...
| 리뷰 | 발견과 그 상태 |
|---|---|
| [2026-08-06](notes/review-2026-08-06.md) | 놀라웠지만 틀린 뉴스 유실 측정 3건 철회; 수집을 GitHub Actions에서 옮기지 않기로 결정 |
| [2026-08-07](notes/review-2026-08-07.md) | H1 등급 아카이브(✅ 08-08 수정) · H2 유령 가중치(✅ 08-08 수정) · M1 뉴스 실패 보고(🟡 부분 해결) · L1 중복 `.gitignore` 줄(⬜ 미해결, 무해) |
| [2026-08-27](notes/review-2026-08-27.md) | **현재 리뷰.** 프로덕션 배선 실패(§1), 나중 세션용 코드 발견 다수(§2), 이번 문서 동기화(§3), 방향성은 건재 + 4가지 유의점(§4), 데이터/시간 대기 항목(§5) |
