...
| 9     | 골든셋 100건 라벨링                       | 약 2시간               | [§4](#4-골든셋--모델-선택을-막는-항목) |
| 10    | KIS 신청 시작 (승인 대기가 며칠 걸림)     | 15분                   | [§1](#1-계정과-인증-정보)     |

**Claude를 막고 있는 것은 이제 없다.** 7·8번은 등급 정확도에 관한 판단이고 급하지 않다 — 없어도 나머지 피처로 등급이 나온다. 9번은 모델 선택 시점에 필요하고, 10번은 아무것도 막지 않는다.
...
| Tiingo                   | ✅ 보유                          | **현재 미사용.** 백필과 일상 수집은 Alpaca로 간다(`scripts/backfill.py`). 교차검증용으로 유지하고, `us_price.py`의 `SCHEMA`·`KNOWN_VALUE`를 Alpaca 수집기가 그대로 가져다 쓴다                                        |
