...
**도구가 있다: `scripts/golden.py`.** 기사 고르기·순서 정하기·버킷 세기·파일 쓰기는 도구가 한다. **Ricky는 읽고 판단만 한다.** 대략 **50분**이면 끝난다.

> **LLM이 라벨을 미리 채우는 일은 없고, 앞으로도 없어야 한다.** 골든셋은 *모델이 얼마나 잘하는지 재는 자*다. 모델이 먼저 채우면 자기 시험지를 자기가 채점하는 꼴이고, 사람은 먼저 본 숫자에 끌려간다. 그러면 점수가 실제보다 좋게 나오고 **그 사실을 나중에 알아챌 방법이 없다.** 도구가 자동화한 것은 전부 잡일뿐이다.

### 순서대로 하면 되는 것

```bash
# 1. 후보 240건 뽑기 — 1초, 한 번만
...
**아무 때나 `q`로 끊어도 된다.** 한 건 판단할 때마다 바로 파일에 쓰이므로, 다시 실행하면 멈춘 자리에서 이어진다.

### 2단계 — 분류 (`triage`)

기사가 하나씩 뜬다. 종목·언론사·날짜·제목·본문 요약·링크가 같이 나온다. 숫자 하나만 누르면 된다.

```
  1 = 명백한 긍정      2 = 명백한 부정
  3 = 애매             4 = 무관 (종목은 언급되나 실적·주가와 무관)
  s = 건너뛰기         q = 저장하고 종료

  진행: 긍정 7/25  부정 3/25  애매 12/25  무관 25/25
```

**버킷이 차면 그 번호는 더 안 받는다.** 25/25/25/25가 되면 자동으로 끝난다.

> **애매·무관을 정직하게 채울 것.** 쉬운 것만 고르면 모든 모델이 다 잘해 보여서 아무것도 구분하지 못한다. `verify`가 이걸 잡아내긴 하지만, 애초에 정직하게 뽑는 게 낫다.
...
```
  ...
  점수 > 0.9 -0.4 0.6 0.3 0.8
```

| 차원 | 범위 | 기준 |
| --- | --- | --- |
| `relevance` | 0~1 | 이 기사가 **그 회사의** 실적·주가에 관계되는가. 업종 전반 논평은 낮게 |
| `polarity` | **−1~1** | **방향만.** 크기는 아래 intensity가 받는다 |
| `intensity` | 0~1 | 재무적 충격의 크기. 대형 수주는 높다. CEO 컨퍼런스 참석은 톤이 좋아도 낮다 |
| `uncertainty` | 0~1 | 그 결과가 실제로 일어날 가능성. MOU는 높다. 체결된 계약은 낮다 |
| `forwardness` | 0~1 | 0 = 이미 주가에 반영된 과거, 1 = 미래 기대를 바꾸는 것 |

범위를 벗어나거나 개수가 안 맞으면 다시 물어본다. **잘못된 값이 파일에 들어가는 일은 없다.**

> ⚠ **그 뒤 주가가 어떻게 됐는지 보지 말고 매길 것.** 사후 정보로 라벨링하면 어떤 모델도 못 맞추는, 그래서 아무것도 측정하지 못하는 골든셋이 나온다.

### 4단계 — 재라벨링 검사 (`recheck`)

**하루 지난 뒤에** 돌린다. 100건 중 10건을 다시 매기는데, **첫 회차 답은 보여주지 않는다** — 보여주면 기억력 시험이 되어버린다. 같은 날 돌리면 도구가 거부한다.

`verify`가 두 회차 평균 차이를 계산한다. **0.25보다 크면** Ricky 자신도 기준이 흔들린다는 뜻이고, 그러면 모델끼리 비교하는 게 의미가 없다 — 스키마부터 조여야 한다.

### 5단계 — 검사 (`verify`)

- 버킷이 25/25/25/25인가
- 중복·결측·범위 이탈이 없는가
- **강한 의견이 충분한가** (|polarity| ≥ 0.5가 25% 미만이면 경고)
- 재라벨링 일치도

전부 통과하면 `✅ 골든셋 정상 — 100건`이 뜬다. 그리고 커밋하면 된다.
