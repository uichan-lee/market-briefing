...
| **9** | **폰에서 브리핑 이메일 열어보기**          | **2분**                | [§13](#13-이제-매일-오는-것) |
| 10    | 골든셋 100건 라벨링                       | 약 2시간               | [§4](#4-골든셋--모델-선택을-막는-항목) |
| 11    | KIS 신청 시작 (승인 대기가 며칠 걸림)     | 15분                   | [§1](#1-계정과-인증-정보)     |

**Claude를 막고 있는 것은 이제 없다.** 7·8번은 등급 정확도에 관한 판단이고 급하지 않다 — 없어도 나머지 피처로 등급이 나온다.

**9번이 지금 가장 중요하다.** 테스트로는 절대 확인할 수 없는 항목이라서다. 이메일 본문을 헤더 + 등급 섹션만으로 줄인 것은 "폰에서 읽힌다"는 *주장*이고, 그 주장은 Ricky가 폰으로 열어봐야만 참·거짓이 갈린다. 안 읽히면 그건 발견이지 실패가 아니다 — 바로 고치면 된다.

10번은 모델 선택 시점에 필요하고, 11번은 아무것도 막지 않는다.
...
지금 받은편지함에 브리핑이 들어와 있다. 제목이 `📅 2026-08-06 (목) …` 형태다.

- [ ] 폰에서 연다
- [ ] **표가 깨지지 않고 읽히는지** 본다
- [ ] 안 읽히면 그대로 말해줄 것

이메일 본문은 마크다운 표다. 메일 앱에 따라 표가 뭉개질 수 있고, 그러면 짧게 줄인 의미가 없다. **이건 코드로 확인이 불가능한 유일한 항목이라서 남겨둔 것이다.**
