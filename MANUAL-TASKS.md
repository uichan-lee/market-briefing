...
| **5** | **Alpaca 키 발급 + 피드 확인**            | **10분**               | [§1](#alpaca--us-시세-소스-교체)  |
| **6** | **`config/aliases.yaml` 작성**            | **60~75분**            | [§3](#3-별칭-사전--blocking)  |
| 7     | KIS 신청 시작 (승인 대기가 며칠 걸림)     | 15분                   | [§1](#1-계정과-인증-정보)     |

**5번과 6번이 Claude를 막고 있다.** 나머지는 아무것도 막지 않는다.

> 5번은 10분이지만 **먼저 하는 게 좋다.** 미국 종목이 40개가 되면서 Tiingo가 한계에 걸렸고(§1 참조), 그 확인 결과에 따라 `us_price` 수집기를 바꿀지 말지가 정해진다. 6번은 시간이 오래 걸리니 5번을 눌러두고 시작하면 된다.

> 4번은 Ricky가 종목 지시를 주고 Claude가 조사·검증해서 채웠다. 5번은 그렇게 할 수 없다 — 별칭은 한국 금융 헤드라인이 회사를 실제로 어떻게 부르는지에 대한 판단이고, CLAUDE.md가 자동 생성을 금지한다. 기계적인 절반은 이미 워크시트로 만들어져 있다.
...
| **Alpaca**               | ⬜ **미완료 — §5**               | 미국 워치리스트가 40종목이 되면서 Tiingo가 한계에 걸렸다. 아래 참조                                                                                                                                                  |
...
#### ⚠ 발급 전에 알아야 할 것 — 확인 하나가 전부를 좌우한다

**Alpaca 문서가 서로 모순된다.** 둘 다 2026-08-04에 읽었다:

- 요금제 비교표: 무료(Basic) 플랜은 주식에 대해 **IEX 전용**
- Market Data FAQ: *과거* 데이터 조회는 `end`가 15분 이상 지난 시점이기만 하면 **SIP** 사용 가능

**이 차이는 사소하지 않다.** IEX는 거래소 하나이고, SIP는 미국 전 거래소를 통합한 체결 정보다. Alpaca 자신의 FAQ가 든 예시 — AAPL 2023-09-29:
...
**56배 차이다.** IEX 데이터를 쓰면 모든 미국 종목의 거래량이 틀리고, 종가는 공식 종가가 아니라 한 거래소의 마지막 체결가가 된다.
...
- **페이퍼 트레이딩 키로 충분하다.** 이 시스템은 주문을 내지 않고, CLAUDE.md 절대 규칙 2가 주문 코드 작성을 금지한다. 페이퍼 키를 쓰면 그게 규율이 아니라 구조가 된다
- **시크릿은 한 번만 보여준다.** 바로 복사할 것. 재발급하면 이전 쌍은 무효가 된다
- 카드 등록 없음
...
SPY의 2024-01-02를 가져온다. **바로 그 날짜의 종가 472.65를 이미 Yahoo Finance로 교차 확인해뒀기 때문에**, 네 번째 검사가 그대로 피드 판별기가 된다.

| 결과 | 의미 |
|---|---|
| 종가 일치 + 거래량 수천만 주 | **SIP.** 교체 진행 |
| 거래량 백만 주 언저리 | **IEX.** `feed` 파라미터가 뭐라 했든 이 플랜으로는 못 쓴다 |
| `sip: REFUSED` | 무료 플랜에 SIP 권한 없음. Tiingo 유지 |

결과를 알려주면 그다음은 Claude가 처리한다.

**3. secrets에 추가**

```bash
set -a; source .env; set +a
for k in ALPACA_API_KEY_ID ALPACA_API_SECRET_KEY; do
  gh secret set "$k" --body "${(P)k}" && echo "set $k"
done
```
