...
def load_filing_ids(path: Path | None = None) -> dict[str, dict[str, dict[str, str]]]:
    """Load ``config/filing_ids.yaml`` — ticker → SEC CIK / DART corp_code.

    Unlike ``aliases.yaml``, this file is not hand-judgment: both mappings come
    from an official, exact-match, government-published enumeration (SEC's
    ``company_tickers.json``, DART's ``corpCode.xml``), regenerated by
    ``scripts/resolve_filing_ids.py`` whenever ``watchlist.yaml`` changes. A
    wrong entry is therefore not the "corrupts silently" risk CLAUDE.md forbids
    auto-generating ``aliases.yaml`` over — the risk here runs the other way:
    a watchlist ticker with **no** entry would make ``us_filings``/
    ``kr_filings`` silently skip that ticker forever, with nothing in the
    report header to say so. So the missing case, not the wrong case, is what
    this function guards — every watchlist ticker must resolve, or loading
    fails loudly naming which one didn't.
    """
    ...
    for market, key in (("KR", "kr"), ("US", "us")):
        ...
        if not isinstance(section, dict):
            raise ConfigError(f"{path.name}: {key!r} section must be a mapping")
        known = {entry.ticker for entry in load_watchlist(market=market)}
        missing = known - section.keys()
        if missing:
            raise ConfigError(
                f"{path.name}: watchlist tickers {sorted(missing)} have no {key} "
                "filing-id entry — rerun scripts/resolve_filing_ids.py"
            )
        id_field = "corp_code" if key == "kr" else "cik"
        for ticker, row in section.items():
            if not isinstance(row, dict) or not row.get(id_field):
                raise ConfigError(f"{path.name}: {key}.{ticker} is missing {id_field!r}")
